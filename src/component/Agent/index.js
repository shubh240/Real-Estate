...
import { getAgentApiCall } from "../../store/slice/agentLenderSlice";
...
const Agent = () => {
    ...
    const handleAgentAPI = () => {
        ...
        dispatch(getAgentApiCall(submitData));
    ...
    return (
        ...
        <main className="page_wrepper">
                ...
                <div className="container-fluid">
                    <div className="row">
                        <div className="col-12 d-flex align-items-baseline justify-content-between pb-30 realeast_headline">
                            <h2 className="headline_txt fs-24">{t("Real Estate Agents")}</h2>
                            <input
                                type="text"
                                name=""
                                ...
                                placeholder={t("Search by agent name or location")}
                            />
                        </div>
                    </div>
                    <div className="row align-items-center">
                        {
                            agentList && agentList.length > 0 ? agentList?.map((e) =>
                                ...
                                <h4 className="d-flex justify-content-center align-items-center mt-5">{t("No_Agents_Found")}</h4>
                        }
                    </div>
                </div>
            </section>
            ...
            {
                agentList.length > 0 && agentList[0]?.per_page < agentList[0]?.agent_count &&
                <Pagination per_page={agentList[0]?.per_page} pageCount={agentList[0]?.agent_count} onPageChange={handlePageChange} page={page} lableName={`Agent`} />
            }
        </main >
    )
}
